...
class TeacherModel(Base):
    ...
    unavailable = relationship(
        "TeacherUnavailableModel", back_populates="teacher",
...
class LessonBlockModel(Base):
    """
    One row = one unique subject/teacher/class/room combination.

    sessions (JSONB): describes the weekly schedule breakdown.
      Free lesson example:  [{"duration": 1, "count": 3}, {"duration": 2, "count": 1}]
      Meaning: 3 single-period + 1 double-period slots per week = 5 periods/week total.

      Locked lesson: sessions = [] — use is_locked + locked_day + locked_start_period + locked_duration instead.

    The mapper explodes sessions into individual GA LessonBlock objects automatically.
    """
    ...
    # sessions: [{duration: int, count: int}, ...]  — empty for locked lessons
    sessions            = Column(JSONB,   nullable=False, default=list)

    # locked lesson fields
